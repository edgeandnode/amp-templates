...
A real-time portfolio tracking application built with Vite, React, Effect Atom, Apache Arrow Flight and Amp. Track ERC20 token balances and transfer tokens with MetaMask wallet integration, powered by the Amp data platform.
...
- Amp Arrow Flight query proxy server (http://localhost:3001)
...
- Test Arrow Flight API and query proxy:
  ```bash
  pnpm amp query 'SELECT * FROM "_/portfolio_dapp@dev"."erc20_transfers" LIMIT 5'
  ```
