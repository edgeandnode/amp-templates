...
export function TransactionHistory({ address }: TransactionHistoryProps) {
  const { transfers, isLoading, isError } = usePortfolioQuery(address as `0x${string}` | undefined)
  const [sorting, setSorting] = useState<SortingState>([{ id: "blockNum", desc: true }])

  const transactions = useMemo(() => {
    // usePortfolioQuery already filters by address, so just return the transfers
    return transfers || []
  }, [transfers])
  ...
  const uniqueTokenAddresses = useMemo(() => {
    ...
    transactions.forEach((transaction) => {
      addressSet.add(`${transaction.contractAddress}`)
    })
    ...
    () => [
      {
        ...
        header: "Block",
        cell: (info) => <span className="font-mono text-sm text-gray-300">{info.getValue() as string}</span>,
      },
      {
        accessorKey: "txHash",
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          const metadata = tokenMetadataMap.get(tokenAddress.toLowerCase())
          const decimals = metadata?.decimals ?? 18 // Default to 18 if metadata not loaded yet

          const amount = BigInt(info.row.original.amountRaw)
  ...
  if (isError) {
    return (
      <div className="rounded-lg border border-red-700 bg-red-900/20 p-4">
        <p className="text-sm text-red-400">Error loading transaction history</p>
      </div>
    )
  }
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-gray-700 bg-gray-800/50">
      <div className="border-b border-gray-700 p-4">
        ...
        <p className="mt-1 text-sm text-gray-400">{transactions.length} transaction(s)</p>
      </div>
