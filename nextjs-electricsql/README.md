...
**Note** it hard-codes the `blocks` table when making the request to the electric-sql endpoint. This can be updated to dynamically pass in the table via a dynamic route param, or individual routes can be created for the tables defined in your [nozzle config](./nozzle.config.ts).
