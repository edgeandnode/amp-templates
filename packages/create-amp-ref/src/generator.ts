...
export async function generateProject(config: ProjectConfig): Promise<void> {
  const { chainId, networkDisplayName, rpcUrl } = getNetworkConfig(config.network, config.networkEnv)
  ...
  if (!config.skipInstall) {
    const installSpinner = ora(`Installing dependencies with ${templateData.packageManager}...`).start()
    try {
...
async function generateFrontend(config: ProjectConfig, data: TemplateData): Promise<void> {
  ...
  // Copy common files
  await copyTemplateDir(path.join(templatesDir, "common", "frontend"), path.join(config.path, "frontend"), data)

  // Copy framework-specific files
  await copyTemplateDir(path.join(templatesDir, config.framework), path.join(config.path, "frontend"), data)
...
async function generateAmpConfig(config: ProjectConfig, data: TemplateData): Promise<void> {
  ...
  if (data.isWalletExample) {
    await copyTemplateDir(path.join(templatesDir, "examples", "wallet", "amp"), path.join(config.path, "amp"), data)
  }
...
async function generateContracts(config: ProjectConfig, data: TemplateData): Promise<void> {
  ...
  // Copy contract files
  await copyTemplateDir(path.join(templatesDir, "contracts"), path.join(config.path, "contracts"), data)
...
async function generateDockerCompose(config: ProjectConfig, data: TemplateData): Promise<void> {
  ...
  // In monorepo: packages/create-amp/dist -> ../../.. to root, then templates/
  const templatesDir = path.join(__dirname, "..", "..", "..", "templates")
  const dockerComposeTemplate = path.join(templatesDir, "docker-compose", `docker-compose.${data.dataLayer}.yml`)
