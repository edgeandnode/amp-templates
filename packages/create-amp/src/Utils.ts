...
/**
 * Resolves a path to an absolute path, handling tilde expansion and cross-platform differences.
 * - If path starts with ~ (Unix/macOS convention), expands to home directory
 * - If path is already absolute, returns normalized path
 * - Otherwise, resolves relative to current working directory
 *
 * @param inputPath The path to resolve (e.g., "~/dev/project", "my-project", or "C:\projects\my-project")
 * @returns Effect that yields the absolute path
 */
export function expandAndResolvePath(inputPath: string): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    const path = yield* Path.Path

    // Handle tilde expansion for Unix/macOS (~ is a shell convention, not Windows)
    if (inputPath.startsWith("~/") || inputPath === "~") {
      const homeDir = os.homedir()
      const expandedPath = inputPath === "~" ? homeDir : homeDir + inputPath.slice(1)
      return path.normalize(expandedPath)
    }

    // If it's already an absolute path (e.g., /home/user/... or C:\Users\...), just normalize
